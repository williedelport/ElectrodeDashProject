...
def run_kmeans_overlay():
    ...
    long_with_cluster = long_df.loc[X_long.index, ["TS"] + long_columns].copy()
    long_with_cluster["cluster"] = cluster_labels
    long_with_cluster["Source"] = "long"

    short_overlay = short_df.loc[X_short.index, ["TS"] + short_columns].rename(columns={v: k for k, v in column_mapping.items()}).copy()
